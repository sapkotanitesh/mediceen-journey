...
export function JourneyPath() {
  ...
  return (
    <>
      {/* Mobile: simple vertical rail — always readable */}
      <ol className="mx-auto w-full max-w-md space-y-4 md:hidden">
        {journeySteps.map((step, i) => (
          <li key={step.label} className="flex gap-4">
            <div className="flex flex-col items-center">
              <span className="mt-1 size-2.5 rounded-full bg-brand" />
              {i < total - 1 ? <span className="w-px flex-1 bg-border" /> : null}
            </div>
            <div className="pb-2">
              <p className="font-display text-sm font-semibold uppercase tracking-[0.16em] text-brand">
                {step.label}
              </p>
      ...
      <div className="relative mx-auto hidden w-full max-w-3xl md:block" data-journey-path>
        <svg
